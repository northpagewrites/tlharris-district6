<?php
/**
 * Title: Homepage — site index
 * Slug: tlharris-public/home-explore
 * Categories: tlharris
 * Inserter: no
 * Description: Plain text links to the main sections of the site.
 */
?>
<!-- wp:heading {"className":"tlharris-keyline"} -->
<h2 class="wp-block-heading tlharris-keyline">Find your way around</h2>
<!-- /wp:heading -->

<!-- wp:html -->
<ul class="tlharris-index">
<li><a href="/about/">About</a><span>His story, background and service</span></li>
<li><a href="/priorities-progress/">Priorities</a><span>The eight issues he is tracking and where each stands</span></li>
<li><a href="/district-6/">District 6</a><span>The 26 schools in the district and how to find yours</span></li>
<li><a href="/board-work/">The Board</a><span>How the board works, meetings and public comment</span></li>
<li><a href="/news/">News</a><span>Updates from the office and coverage of his work</span></li>
</ul>
<!-- /wp:html -->
